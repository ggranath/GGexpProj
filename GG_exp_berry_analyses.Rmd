...
# next model we controll for prcent plant cover. With the offset term we get fruits per percent cover
...
#sums2 <- sums[!(sums$exp_name=="929Nysund"),] #if removed an effect of latitude
bill.he.mc <- MCMCglmm(billberry_herbivory  ~ year*N*thin + year*P + Latitud.s + log(billberry_cover), 
                       random = ~idh(year):exp_name + id, data=sums, family = "gaussian",  
                       nitt = 80000, burnin = 15000, thin=25, 
                       pr = TRUE, pl = TRUE, saveX = TRUE,  saveZ = TRUE, prior=prior)
summary(bill.he.mc) # NO significant effects
...
cow.he.mc <- MCMCglmm(cowberry_fungi  ~ year*N*thin + year*P + Latitud.s + log(cowberry_cover+0.5), 
                      random = ~exp_name + id, data=sums, family = "gaussian",  
                      nitt = 80000, burnin = 15000, thin=25, 
                      pr = TRUE, pl = TRUE, saveX = TRUE,  saveZ = TRUE, prior=prior)
...
cow.corr.mc <- MCMCglmm(cowberry_fruits  ~ cowberry_cover*year + cowberry_fungi*year + cowberry_herbivory*year, random = ~idh(year):exp_name + id, data=sums, family = "poisson",  nitt = 80000, burnin = 15000, thin=25, pr = TRUE, pl = TRUE, saveX = TRUE,  saveZ = TRUE, prior=prior)
summary(cow.corr.mc)
